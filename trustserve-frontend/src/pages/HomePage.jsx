...
function HomePage() {
  ...
  function useCurrentLocation() {
    ...
    setLocationStatus('Please allow location access in your browser prompt...')
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          if (error.code === error.PERMISSION_DENIED) {
            setLocationStatus('Location permission denied. Please allow it in browser settings.')
          } else if (error.code === error.POSITION_UNAVAILABLE) {
            setLocationStatus('Location is unavailable right now. Please try again.')
      ...
      navigator.permissions
        ...
        .then((permissionStatus) => {
          if (permissionStatus.state === 'denied') {
            setLocationStatus('Location permission is blocked. Enable it in browser settings.')
            setIsRequestingLocation(false)
            return
          }

          if (permissionStatus.state === 'prompt') {
            setLocationStatus('Browser will ask for location permission. Please tap Allow.')
          }
  ...
  function handleDenyLocationPermission() {
    setShowLocationPermissionModal(false)
    setLocationStatus('Location access denied. You can enter location manually.')
  }
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        setRecommended([])
        setRecommendationError('Recommended workers are unavailable right now.')
      } finally {
      ...
      try {
        ...
        setTopRated([])
        setTopRatedError('Top rated workers are unavailable right now.')
      } finally {
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
          ...
          <div className="relative z-10">
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-3">
              <div className="rounded-xl border border-slate-700 bg-slate-900/70 px-4 py-3">
                ...
                <p className="text-xs text-slate-300">Verified workers</p>
              </div>
              <div className="rounded-xl border border-slate-700 bg-slate-900/70 px-4 py-3">
              ...
              <div className="rounded-xl border border-slate-700 bg-slate-900/70 px-4 py-3">
                ...
                <p className="text-xs text-slate-300">Secure bookings</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {showLocationPermissionModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-neutral-950/50 px-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-neutral-200 bg-white p-6 shadow-2xl dark:border-neutral-800 dark:bg-neutral-900">
            <h3 className="font-heading text-lg font-semibold text-neutral-900 dark:text-white">Enable location access?</h3>
      ...
      {showLoginRequiredModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-neutral-950/50 px-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-neutral-200 bg-white p-6 shadow-2xl dark:border-neutral-800 dark:bg-neutral-900">
            <h3 className="font-heading text-lg font-semibold text-neutral-900 dark:text-white">Sign in required</h3>
            <p className="mt-2 text-sm text-neutral-600 dark:text-neutral-400">
              Create an account or sign in to book a service.
            </p>
            <div className="mt-6 flex gap-3">
              <button
                type="button"
      ...
      <AnimatedSection className="mt-12 rounded-2xl border border-neutral-200 bg-gradient-to-r from-primary-50 to-accent-50 p-8 transition-colors dark:border-neutral-800 dark:from-primary-900/20 dark:to-accent-900/20">
        <div className="flex flex-col items-start justify-between gap-6 lg:flex-row lg:items-center">
          <div className="flex-1">
            <p className="mb-2 text-sm font-semibold uppercase tracking-wider text-primary-700 dark:text-primary-300">Earn & Grow</p>
            <h2 className="font-heading text-3xl font-bold text-neutral-900 dark:text-white">Become a Service Provider</h2>
      ...
      <AnimatedSection className="mt-12 rounded-3xl border border-slate-800 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-900 p-6 sm:p-8">
        <div className="mb-8">
          <h2 className="font-heading text-3xl font-bold text-white">Why Choose TrustServe?</h2>
        </div>
        <div className="grid gap-4 sm:grid-cols-3">
          <article className="rounded-2xl border border-slate-700 bg-slate-900/80 p-6 shadow-sm transition-all hover:border-slate-600 hover:shadow-md">
            ...
            <h3 className="font-heading font-semibold text-white">Verified & Safe</h3>
          ...
          <article className="rounded-2xl border border-slate-700 bg-slate-900/80 p-6 shadow-sm transition-all hover:border-slate-600 hover:shadow-md">
            ...
            <h3 className="font-heading font-semibold text-white">Neighborhood Discovery</h3>
            <p className="mt-2 text-sm text-slate-300">
          ...
          <article className="rounded-2xl border border-slate-700 bg-slate-900/80 p-6 shadow-sm transition-all hover:border-slate-600 hover:shadow-md">
            ...
            <h3 className="font-heading font-semibold text-white">Transparent Reviews</h3>
            <p className="mt-2 text-sm text-slate-300">
      ...
      <AnimatedSection className="mt-12">
        <h2 className="mb-8 font-heading text-3xl font-bold text-neutral-900 dark:text-white">Recommended For You</h2>
        <div className="grid gap-4 md:grid-cols-2">
          ...
          {!loadingRecommendations && recommendationError &&
            <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-5 text-sm text-amber-800 dark:border-amber-700/30 dark:bg-amber-900/20 dark:text-amber-200 md:col-span-2">
              {recommendationError} Showing featured professionals.
            </div>}
          ...
          {!loadingRecommendations && !recommendationError && !recommended.length &&
            <div className="rounded-xl border border-neutral-200 bg-white px-4 py-5 text-sm text-neutral-600 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-400 md:col-span-2">
              Showing featured professionals in your area.
            </div>}
      ...
      {/* Top Rated Section */}
      <AnimatedSection className="mt-12">
        <h2 className="mb-8 font-heading text-3xl font-bold text-neutral-900 dark:text-white">Top Rated Professionals</h2>
      ...
      <AnimatedSection className="mt-12 grid gap-6 lg:grid-cols-2">
        <section className="rounded-2xl border border-slate-700 bg-slate-900/80 p-8 shadow-sm transition-colors">
          <h2 className="font-heading text-2xl font-bold text-white">Looking for Services?</h2>
        ...
        <section className="rounded-2xl border border-slate-700 bg-slate-900/80 p-8 shadow-sm transition-colors">
          <h2 className="font-heading text-2xl font-bold text-white">Want to Earn More?</h2>
          <p className="mt-3 text-slate-300">
